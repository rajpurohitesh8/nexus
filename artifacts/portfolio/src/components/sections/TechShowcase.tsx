...
const LOGOS = "React • TypeScript • Next.js • Node.js • Python • Go • Rust • AWS • GCP • Docker • Kubernetes • PostgreSQL • Redis • TensorFlow • GraphQL • React Native • ";
...
export function TechShowcase() {
  ...
  return (
    <section id="tech" className="py-32 relative bg-background border-t border-white/5 overflow-hidden">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            >
              ...
              <span className="text-xs font-semibold tracking-widest uppercase text-white/80">STACK & EXPERTISE</span>
            </motion.div>
            ...
            >
              Built With <span className="text-gradient-primary">Precision</span>
            </motion.h2>
            ...
            >
              Every line of code is deliberate. Every architecture decision is battle-tested. We employ the most advanced technology stack to ensure scalability, security, and exceptional performance.
            </motion.p>
          ...
          >
            <div className="rounded-xl overflow-hidden bg-[#0d0d0d] border border-white/10 shadow-2xl relative">
              ...
              <div className="bg-[#1a1a1a] px-4 py-3 flex items-center border-b border-white/5">
                ...
                <div className="mx-auto text-xs font-mono text-white/40">nexus-deploy.sh</div>
              </div>
              ...
              <div className="p-6 font-mono text-sm md:text-base h-[350px] overflow-hidden relative terminal-scanlines flex flex-col justify-end pb-8">
                <div className="flex flex-col justify-end w-full">
                  {displayedLines.map((line, i) => (
                    <div key={i} className="mb-2 w-full break-words whitespace-pre-wrap">
                      {line.startsWith('$') ? (
                        <div>
                          <span className="text-[#22c55e] mr-2">root@nexus:~$</span>
                          <span className="text-[#22d3ee]">{line.substring(2)}</span>
                        </div>
                      ) : (
                        <div className="text-[#22c55e]">
                          {line.split(/(✓|\d+(?:\.\d+)?%?|100\/100|\[={10}\])/).map((part, j) => {
                            if (part === '✓') return <span key={j} className="text-white font-bold">{part}</span>;
                            if (part === '[==========]') return <span key={j} className="text-[#8b5cf6]">{part}</span>;
                            if (/^\d/.test(part)) return <span key={j} className="text-white font-bold">{part}</span>;
