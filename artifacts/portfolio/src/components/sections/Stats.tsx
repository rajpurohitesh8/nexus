...
const STATS = [
  ...
  { value: 2, label: "Revenue Generated", suffix: "B+", prefix: "$", icon: TrendingUp },
];
...
function Counter({ from, to, prefix = "", suffix }: { from: number; to: number; prefix?: string; suffix: string }) {
  ...
  return (
    <span ref={nodeRef} className="text-5xl lg:text-6xl font-display font-bold text-white tracking-tighter drop-shadow-lg">
      {prefix}{count}{suffix}
    </span>
  );
}
