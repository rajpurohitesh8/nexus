...
const STATS = [
  ...
  { value: 99.97, prefix: "", suffix: "%", label: "Average Uptime", desc: "Across all client production systems", decimals: 2 },
  { value: 847, prefix: "", suffix: "K", label: "Peak Requests/Sec", desc: "Handled without performance degradation" },
  { value: 2.4, prefix: "$", suffix: "B+", label: "Client Revenue Influenced", desc: "Through platforms we've engineered", decimals: 1 },
  { value: 2847, prefix: "", suffix: "", label: "Test Cases Per Project", desc: "Zero-compromise quality assurance" },
  { value: 12, prefix: "", suffix: "ms", label: "Average API Response", desc: "Sub-20ms guaranteed SLA on all builds" },
];

function ComplexCounter({ stat }: { stat: any }) {
  ...
  const displayValue = stat.decimals ? count.toFixed(stat.decimals) : Math.floor(count).toLocaleString();
